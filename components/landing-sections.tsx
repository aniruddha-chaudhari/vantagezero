    ...
    <>
      <section className="border-t border-black/[0.06] bg-[#f8f8f6] px-6 py-24 sm:py-32 lg:py-40">
        <div className="mx-auto grid max-w-[1320px] gap-14 lg:grid-cols-[0.72fr_1.28fr] lg:gap-20">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-black/38">Buildability layer</p>
            ...
            <p className="mt-7 max-w-[470px] text-base leading-7 text-black/54 sm:text-lg sm:leading-8">
          ...
          <div className="overflow-hidden rounded-[22px] border border-black/[0.08] bg-white shadow-[0_28px_80px_rgba(0,0,0,0.055)]">
            <div className="flex flex-col gap-8 border-b border-black/[0.06] px-6 py-7 sm:flex-row sm:items-end sm:justify-between sm:px-8 sm:py-8">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-black/34">Buildable quantity</p>
                <div className="mt-3 flex items-baseline gap-3">
                  <span className="font-display text-6xl tracking-[-0.055em] sm:text-7xl">820</span>
                  <span className="text-sm font-medium text-black/38">/ 1,000 units</span>
                </div>
              </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[780px] border-collapse text-left">
                <thead>
                  <tr className="border-b border-black/[0.055] text-[10px] font-semibold uppercase tracking-[0.11em] text-black/34">
                ...
                <tbody>
                  {bomRows.map(([part, need, stock, coverage, lead, lifecycle, risk]) => (
                    <tr key={part} className="border-b border-black/[0.045] last:border-b-0">
                      ...
                      <td className="px-4 py-5 text-xs text-black/48">{need}</td>
                      ...
                      <td className="px-4 py-5 text-xs text-black/48">{lead}</td>
                      <td className="px-4 py-5 text-xs text-black/48">{lifecycle}</td>
            ...
            <div className="flex flex-col gap-2 border-t border-black/[0.055] bg-black/[0.012] px-6 py-5 text-xs text-black/44 sm:flex-row sm:items-center sm:justify-between sm:px-8">
      ...
      <section id="how-it-works" className="scroll-mt-24 border-t border-black/[0.06] bg-white px-6 py-24 sm:py-32 lg:py-40">
        <div className="mx-auto max-w-[1320px]">
          <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-black/38">How it works</p>
          ...
          <div className="mt-16 grid border-y border-black/[0.07] lg:grid-cols-3 lg:divide-x lg:divide-black/[0.07]">
            {workflow.map(([number, title, body], index) => (
              <div key={number} className={`py-10 lg:px-10 lg:py-12 ${index < workflow.length - 1 ? "border-b border-black/[0.07] lg:border-b-0" : ""}`}>
                <span className="font-mono text-[11px] font-medium text-black/28">{number}</span>
                <h3 className="mt-12 text-[22px] font-semibold tracking-[-0.035em] text-black/82">{title}</h3>
                <p className="mt-4 max-w-[340px] text-sm leading-6 text-black/48">{body}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl border border-black/[0.065] bg-[#f8f8f6] p-6 sm:p-7">
              ...
              <p className="mt-3 text-sm leading-6 text-black/48">DOM moved, label renamed, page template changed → diagnose extraction → verify repair.</p>
            </div>
            <div className="rounded-2xl border border-black/[0.065] bg-[#f8f8f6] p-6 sm:p-7">
              <p className="text-sm font-semibold text-black/72">Semantic change</p>
              <p className="mt-3 text-sm leading-6 text-black/48">Stock fell, lead time moved, lifecycle changed → recompute BOM → surface production impact.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="demo" className="scroll-mt-24 border-t border-black/[0.06] bg-[#f8f8f6] px-6 py-24 sm:py-32 lg:py-40">
        <div className="mx-auto max-w-[1320px]">
          <div className="max-w-[800px]">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-black/38">Live operating view</p>
            ...
            <p className="mt-7 max-w-[640px] text-base leading-7 text-black/52 sm:text-lg sm:leading-8">
          ...
          <div className="mt-16 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
            <div className="rounded-[22px] border border-black/[0.075] bg-white p-6 sm:p-8">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.14em] text-black/34">Event feed</p>
                  <p className="mt-2 text-sm text-black/46">Latest system and supply-chain changes</p>
                </div>
                <span className="flex items-center gap-2 text-[11px] font-medium text-black/42"><span className="h-1.5 w-1.5 rounded-full bg-[#2d6b4a]" />Live</span>
              </div>

              <div className="mt-8">
                {events.map(([time, title, detail], index) => (
                  <div key={`${time}-${title}`} className="grid grid-cols-[56px_16px_1fr] gap-3">
                    <span className="pt-1 font-mono text-[10px] text-black/28">{time}</span>
                    ...
                    <div className="pb-7">
                      ...
                      <p className="mt-1 text-xs leading-5 text-black/42">{detail}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="overflow-hidden rounded-[22px] border border-black/[0.075] bg-white shadow-[0_24px_70px_rgba(0,0,0,0.045)]">
              <div className="border-b border-black/[0.06] px-6 py-6 sm:px-8">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p className="mt-2 text-xs text-black/40">Texas Instruments · Linear regulator</p>
                  </div>
                  <span className="text-xs font-medium text-black/40">Updated 18 sec ago</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4">
                {[["Observed stock", "1,640"], ["Need", "2,000"], ["Shortfall", "360"], ["Coverage", "0.82×"]].map(([label, value]) => (
                  <div key={label} className="border-b border-r border-black/[0.055] p-5 last:border-r-0 sm:p-6">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.11em] text-black/30">{label}</p>
              ...
              <div className="grid gap-8 px-6 py-8 sm:px-8 lg:grid-cols-2">
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-black/30">Observed state</p>
                ...
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-black/30">Evidence chain</p>
      ...
      <section className="border-t border-black/[0.06] bg-white px-6 py-24 sm:py-32 lg:py-40">
        <div className="mx-auto max-w-[1320px]">
          <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr]">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-black/38">Built for decisions</p>
            ...
            <div className="grid gap-x-12 gap-y-10 sm:grid-cols-2">
              {principles.map(([title, body]) => (
                <div key={title} className="border-t border-black/[0.08] pt-6">
                  ...
                  <p className="mt-3 text-sm leading-6 text-black/48">{body}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="border-t border-black/[0.06] bg-black px-6 py-24 text-white sm:py-32 lg:py-36">
        <div className="mx-auto max-w-[1320px] text-center">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/42">Vantage</p>
          ...
          <p className="mx-auto mt-8 max-w-[560px] text-base leading-7 text-white/52 sm:text-lg sm:leading-8">
      ...
      <footer className="border-t border-white/10 bg-black px-6 py-8 text-white/40">
